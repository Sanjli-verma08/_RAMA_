...
const messages = {
	...
	"result": "your prescription is ready. ",
}
...
function insertMessage() {
	...
	switch(msgStep) {
		...
		case 'feeling':
			...
			break;
		case 'not_well':
			let not_well = msg.trim();
			manageAPICall(not_well);
			break;
	}
...
async function manageAPICall(msg) {
	...
	prescription = jsonData;
	trueMessage(messages, 'result')
}
...
function trueMessage(_msg_from_server, type) {
	...
	updateScrollbar();
	const presLink = '<button class="btn btn-sm btn-default" type="button" onClick="createPres()">Click Here</button>'
	setTimeout(function () {
		$('.message.loading').remove();

		if (state == 0 || state == 2)
			$('<div class="message new"><figure class="avatar"><img src="static/res/botim_g.png" /></figure>' + _msg_from_server[type] + (prescription ? presLink : '') + '</div>').appendTo($('.mCSB_container')).addClass('new');
		else
			$('<div class="message new"><figure class="avatar"><img src="static/res/botim.png" /></figure>' + _msg_from_server[type] + (prescription ? presLink : '') + '</div>').appendTo($('.mCSB_container')).addClass('new');
...
function createPres() {
	...
	pre.innerHTML = JSON.stringify(prescription, null, "\t")
	//$("#overlay").show()'
...
function setBG() {
	...
	$('html').css({
		'background-image': 'url(static/res/' + images[Math.floor(Math.random() * images.length)] + ')'
	});
}
